...
void i2c_master_setup(void) {
    ...
    I2C1CONbits.ON = 1; // turn on the I2C1 moduleint IODIRA = 0x00;
}
...
void readMultiplePins(unsigned char address, unsigned char reg, unsigned char * data, int length){
   // unsigned char data[length];
    ...
    for(i=0; i<length+1; i++){
        ...
        if (i < length){
            i2c_master_ack(0); // continue reading i2c
        }
        else{
