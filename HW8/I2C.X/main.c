...
//    setPin(write_add, regIODIRB, valIODIRB); 
    ...
    i2c_master_setup();
    unsigned char write_add = 0b01000000;
    unsigned char regIODIRA = 0x00; // SFR address for IODIRA
    unsigned char regIODIRB = 0x01; // SFR address for IODIRB
    unsigned char valIODIRA = 0x00; // Set A pins as outputs
    unsigned char valIODIRB = 0xFF; // Set B pins as inputs
    
    setPin(write_add, regIODIRA, valIODIRA); 
    setPin(write_add, regIODIRB, valIODIRB);
    
    // Setting pin A addresses and SFR addresses
    unsigned char regOLATA = 0x14;
    unsigned char valOLATAon = 0x80;
    unsigned char valOLATAoff = 0x00;
    
    // Setting pin B addresses and SFR addresses
    unsigned char regGPIOB = 0x13;
    setPin(write_add, regOLATA, valOLATAoff);

    while (1) {
        
        // setPin(write_add, regOLATA, valOLATAon);  
        if( readPin(write_add, regGPIOB) == 0){
            setPin(write_add, regOLATA, valOLATAon);       
        }
        else {
            setPin(write_add, regOLATA, valOLATAoff);  
