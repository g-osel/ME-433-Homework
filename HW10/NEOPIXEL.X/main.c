...
#include <stdio.h>
#include <math.h>
#include "ws2812b.h" // Include color library
...
//    wsColor test;
//    test = HSBtoRGB(270, 0.9, 0.005);
//    ws2812b_setColor(&test, 4);
    ...
    float count3 = 120;
    wsColor colors[4];
    //create heartbeat light to indicate if pic is functioning
    ...
    while (1) {
        ...
        colors[0] = HSBtoRGB(count1, 1, .05);
        colors[1] = HSBtoRGB(count2, 1, .05);
        colors[2] = HSBtoRGB(count3, 1, .05);
        colors[3] = HSBtoRGB(count4, 1, .05);
...
//        ws2812b_setColor(&colors[3], 3);
//        ws2812b_setColor(&colors[2], 2);
//        ws2812b_setColor(&colors[1], 1);
...
void delay(){
            ...
            _CP0_SET_COUNT(0);
            time = _CP0_GET_COUNT();
            while(_CP0_GET_COUNT() - time < 60000){
